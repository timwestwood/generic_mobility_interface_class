...
rpy_mobility_solver::rpy_mobility_solver(){
  ...
  for (int n = 0; n < 3*NSWIM*NBLOB; n++){
    ...
    f_blobs_host[n] = 0.0;
      ...
      std::ifstream generalised_force_file(reference_generalised_force_file_name());

      if (generalised_force_file.good()){

        int num_saves;
        generalised_force_file >> num_saves;
        gen_force_refs = std::vector<double>(num_saves);
        for (int n = 0; n < num_saves; n++){

          generalised_force_file >> gen_force_refs[n];
        ...
        std::cout << std::endl;
        std::cout << std::endl;
        std::cout << "Required reference file for generalised forces was not found." << std::endl;
...
void rpy_mobility_solver::read_positions_and_forces(std::vector<swimmer>& swimmers){
  ...
  void rpy_mobility_solver::assemble_rhs(const std::vector<swimmer>& swimmers, const int nt){
      ...
      for (int n = 0; n < NSWIM; n++){
        ...
        for (int i = 0; i < NFIL; i++){
            ...
            double phi_index = 0.5*swimmers[n].filaments[i].phase/PI; // = phase/(2*pi)
            phi_index -= std::floor(phi_index); // Map this ratio into [0,1]
            phi_index *= gen_force_refs.size();

            double q;

            /* // Nearest-neighbour interpolation for initial testing
            int phi_index_int = std::round(phi_index);
            if (phi_index_int == gen_force_refs.size()){
              phi_index_int = 0;
            }
            q = gen_force_refs[phi_index_int]; */

            /* // Linear interpolation
            int phi_index_int_lower_bound = int(phi_index); // Rounds towards 0.

            if (phi_index_int_lower_bound == gen_force_refs.size()){

              // This can only be true if phi_index == gen_force_refs.size()
              ...
              const double q_lower = gen_force_refs[phi_index_int_lower_bound];
              const double q_upper = (phi_index_int_lower_bound == gen_force_refs.size() - 1) ? gen_force_refs[0] : gen_force_refs[phi_index_int_lower_bound+1];
              q = q_lower + (q_upper - q_lower)*(phi_index - phi_index_int_lower_bound);
            ...
            if (phi_index_int_lower_bound == gen_force_refs.size()){
              ...
              // Otherwise, we're safely in the interior
              const int phi_index_int_upper_bound = (phi_index_int_lower_bound == gen_force_refs.size() - 1) ? 0 : (phi_index_int_lower_bound + 1);

              const double q_lower = gen_force_refs[phi_index_int_lower_bound];
              const double q_prime_lower = (phi_index_int_lower_bound == 0) ? (gen_force_refs[1] - gen_force_refs[0]) : 0.5*(gen_force_refs[phi_index_int_upper_bound] - gen_force_refs[phi_index_int_lower_bound-1]);

              const double q_upper = gen_force_refs[phi_index_int_upper_bound];
              const double q_prime_upper = (phi_index_int_upper_bound == 0) ? (gen_force_refs[0] - gen_force_refs[gen_force_refs.size()-1])
                                                                            : 0.5*(gen_force_refs[(phi_index_int_upper_bound == gen_force_refs.size()-1) ? 0 : phi_index_int_upper_bound+1] - gen_force_refs[phi_index_int_lower_bound]);
            ...
            q *= 0.5*swimmers[n].filaments[i].omega0/PI;

            // Store minus the generalised force in the RHS
            #if PRESCRIBED_BODY_VELOCITIES

              rhs(3*NSWIM*(NFIL*NSEG + NBLOB) + n*NFIL + i) = -q;

            #else

              rhs(3*NSWIM*(NFIL*NSEG + NBLOB + 2) + n*NFIL + i) = -q;
            ...
            for (int m = 0; m < 3*NSEG; m++){
              ...
              rhs(index + m) = swimmers[n].filaments[i].phase_speed*swimmers[n].filaments[i].vel_dir[m];
      ...
      for (int n = 0; n < 3*NSWIM*NBLOB; n++){
        ...
        const double slip_vel_mag = 1.0/(6.0*PI*MU*RBLOB);
        for (int n = 0; n < NSWIM; n++){
          for (int m = 0; m < NBLOB; m++){

            matrix r = matrix(3, 1, &swimmers[n].body.blob_references[3*m]);
            r /= norm(r);

            matrix polar_dir = r(2)*r; // same as = -dot(r,polar_dir)*r if polar_dir starts as -e_z
            polar_dir(2) -= 1.0;
            polar_dir /= norm(polar_dir) + 1e-10; // Add 1e-10 so I don't have to worry about the 0 possibility.

            rhs.add_to_block(3*(n*NBLOB + m), 3, slip_vel_mag*std::sqrt(1.0 - r(2)*r(2))*polar_dir); // sin(theta) scaling
            //rhs.add_to_block(3*(n*NBLOB + m), 3, slip_vel_mag*double(std::abs(r(2)) <= 1.0)*polar_dir); // constant magnitude in an equatorial band
  ...
  matrix rpy_mobility_solver::apply_preconditioner(const matrix& in, const std::vector<swimmer>& swimmers){
          ...
          for (int n = 0; n < NSWIM; n++){
            ...
            for (int m = 0; m < NFIL; m++){
              ...
              for (int k = 0; k < 3*NSEG; k++){
                ...
                temp += swimmers[n].filaments[m].vel_dir[k]*out(3*NSEG*(n*NFIL + m) + k);
                Knorm += swimmers[n].filaments[m].vel_dir[k]*swimmers[n].filaments[m].vel_dir[k];

              }

              out(3*NSWIM*(NBLOB + NFIL*NSEG) + n*NFIL + m) -= temp;
              out(3*NSWIM*(NBLOB + NFIL*NSEG) + n*NFIL + m) /= Knorm*seg_mob_inv;

              for (int k = 0; k < 3*NSEG; k++){

                out(3*NSEG*(n*NFIL + m) + k) += seg_mob_inv*out(3*NSWIM*(NBLOB + NFIL*NSEG) + n*NFIL + m)*swimmers[n].filaments[m].vel_dir[k];
          ...
          for (int m = 0; m < NBLOB; m++){
            ...
            for (int m = 0; m < NFIL; m++){
              ...
              for (int k = 0; k < NSEG; k++){
                ...
                matrix K(3,1);
                K(0) = swimmers[n].filaments[m].vel_dir[3*k];
                K(1) = swimmers[n].filaments[m].vel_dir[3*k + 1];
                K(2) = swimmers[n].filaments[m].vel_dir[3*k + 2];
          ...
          for (int m = 0; m < NFIL; m++){
            ...
            for (int k = 0; k < NSEG; k++){
              ...
              out(seg_force_id + 2) += seg_mob_inv*(out(out_id + 2) + out(out_id + 3)*diff[1] - out(out_id + 4)*diff[0]);
                ...
                out(seg_force_id) += seg_mob_inv*out(phase_id)*swimmers[n].filaments[m].vel_dir[3*k];
                out(seg_force_id + 1) += seg_mob_inv*out(phase_id)*swimmers[n].filaments[m].vel_dir[3*k + 1];
                out(seg_force_id + 2) += seg_mob_inv*out(phase_id)*swimmers[n].filaments[m].vel_dir[3*k + 2];
  ...
  matrix rpy_mobility_solver::system_matrix_mult(const matrix& in, const std::vector<swimmer>& swimmers){
    ...
    matrix out(rhs.num_rows, 1);
      ...
      for (int n = 0; n < num_gpus; n++){
        ...
        Mss_mult<<<num_thread_blocks, THREADS_PER_BLOCK>>>(v_segs_device[n], f_segs_device[n], x_segs_device[n], start_seg, num_segs[n]);
          ...
          num_thread_blocks = (num_blobs[n] + THREADS_PER_BLOCK - 1)/THREADS_PER_BLOCK;
        ...
        for (int n = 0; n < NSWIM; n++){
          ...
          for (int m = 0; m < NFIL; m++){
            ...
            for (int k = 0; k < 3*NSEG; k++){
              ...
              // "-K" mult
              out(out_pos + k) -= phi_dot*swimmers[n].filaments[m].vel_dir[k];

              // "-K^T" mult
              out(in_pos) -= in(out_pos + k)*swimmers[n].filaments[m].vel_dir[k];
  ...
  int rpy_mobility_solver::solve_linear_system(std::vector<swimmer>& swimmers){
        ...
        for (int n = 0; n < 3*NSWIM*NBLOB; n++){
          ...
          for (int n = 0; n < NSWIM; n++){
            ...
            for (int m = 0; m < NFIL; m++){
              ...
              swimmers[n].filaments[m].phase_speed = 0.0;
      ...
      for (int n = 0; n < 3*NSWIM*NBLOB; n++){
        ...
        for (int n = 0; n < NSWIM; n++){
          ...
          for (int m = 0; m < NFIL; m++){
            ...
            soln(in_pos) = swimmers[n].filaments[m].phase_speed;
          ...
          for (int n = 0; n < 3*NSWIM*NBLOB; n++){
            ...
            for (int n = 0; n < NSWIM; n++){
              ...
              for (int m = 0; m < NFIL; m++){
                ...
                swimmers[n].filaments[m].phase_speed = soln(in_pos);
...
void rpy_mobility_solver::compute_velocities(std::vector<swimmer>& swimmers, int& num_gmres_iterations, const int nt){
  ...
  read_positions_and_forces(swimmers);
    ...
    for (int n = 0; n < num_gpus; n++){
      ...
      const int num_thread_blocks = (num_segs[n] + THREADS_PER_BLOCK - 1)/THREADS_PER_BLOCK;
...
bool rpy_mobility_solver::compute_errors(matrix& error, const std::vector<swimmer>& swimmers, const int nt){
  ...
  void rpy_mobility_solver::make_body_reference_matrices(std::vector<swimmer>& swimmers){
    ...
    std::cout << std::endl << "Percentage error in (K^T * K)^(-1) * (K^T * K) is " << 100.0*norm(KTKinv_reference*KTK - identity(6))/sqrt(6.0) << "%" << std::endl;
    ...
    for (int n = 0; n < 6; n++){
      ...
      const double R = double(R_OVER_L)*DL*NSEG;
      matrix Ntrue(6, 6);
      Ntrue.zero();
...
void rpy_mobility_solver::write_data(const int nt, const std::vector<swimmer>& swimmers){
