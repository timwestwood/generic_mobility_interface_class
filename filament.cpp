...
void filament::initial_setup(const double *const base_pos,
  ...
  strain_twist[2] = strain_twist_in[2];
    ...
    int p = 0;
      ...
      vel_dir = std::vector<double>(3*NSEG);
    ...
    qtemp.sqrt_in_place();
        ...
        omega0 = 2.0*PI; // T = 1
          ...
          phase = 2.0*PI*base_pos[0]/(DL*(NSEG-1)) + 2.0*PI*116.0/300.0;

        #elif SPHEROID_BODIES

          const double phi = atan2(base_pos[1], base_pos[0]);

          // Make the wavelength L
          const double C_over_L = 2.0*PI*R_OVER_L; // = number of waves per band
          phase = phi*C_over_L;
          phase -= 2.0*PI*std::floor(0.5*phase/PI);
      ...
      /*  // Fixed perturbations for testing
...
void filament::accept_state_from_rigid_body(const double *const x_in, const double *const u_in){
  ...
  segments[0].x[2] = x_in[2];
    ...
    void filament::find_fitted_shape_s(){
      // TODO: Replace this with something that interpolates over a table saved in a data file, and only does these calculations
...
void filament::initial_guess(const int nt, const double *const x_in, const double *const u_in){
  ...
  accept_state_from_rigid_body(x_in, u_in);
    ...
    phase_speed = omega0; // Serves as the definition if the phase dynamics are prescribed, or an initial guess otherwise.
    ...
    vel_dir[2] = 0.0;
      ...
      const double L = DL*(NSEG - 1); // Different definition of length for the prescribed-shape filaments.
    ...
    for (int n = 1; n < NSEG; n++){
        ...
        const matrix diff = L*R*fitted_shape(s_to_use[n]);

        segments[n].x[0] = segments[0].x[0] + diff(0);
        segments[n].x[1] = segments[0].x[1] + diff(1);
        segments[n].x[2] = segments[0].x[2] + diff(2);

        const matrix dir = L*R*fitted_shape_velocity_direction(s_to_use[n]);
        vel_dir[3*n] = dir(0);
        vel_dir[3*n + 1] = dir(1);
        vel_dir[3*n + 2] = dir(2);
        ...
        vel_dir[3*n] = vel_dir[3*(n-1)] + 0.5*DL*(k1(0) + k2(0));
        vel_dir[3*n + 1] = vel_dir[3*(n-1) + 1] + 0.5*DL*(k1(1) + k2(1));
        vel_dir[3*n + 2] = vel_dir[3*(n-1) + 2] + 0.5*DL*(k1(2) + k2(2));
...
void filament::write_backup(std::ofstream& data_file) const {
  ...
  std::string reference_generalised_force_file_name(){

    #if FULFORD_AND_BLAKE_BEAT

      return std::string("fulford_and_blake_reference_generalised_forces.dat");

    #elif CORAL_LARVAE_BEAT

      return std::string("coral_larvae_reference_generalised_forces.dat");
      ...
      filename_stringstream << "_reference_generalised_forces.dat";
