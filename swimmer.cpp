...
void swimmer::initial_setup(const int id, const double *const data_from_file, double *const x_segs_address, double *const f_segs_address, double *const f_blobs_address){
  ...
  f.zero();
    ...
    // We don't have all of the staggered grids when the 'filament' motion is prescribed.
    // This seems to be the most sensible way of defining length in this case.
    const double L = DL*(NSEG - 1);

  #else

    const double L = DL*NSEG;
      ...
      const double fil_grid_step_x = 1.4*L; //(0.1 + SCALED_BEAT_AMPLITUDE)*L; //2.0*L*sqrt(PI/5.6);
      ...
      const double fil_grid_step_y = 0.5*(1.0122*L + DL); // Gives us beat-wise separations of 2*fil_grid_step_y
      const double fil_grid_step_x = 2.0*fil_grid_step_y/std::sqrt(3.0);
    ...
    const double height_of_base_above_surface = 0.5*DL;
      ...
      for (int i = 0; i < fil_grid_dim_x; i++){
        for (int j = 0; j < fil_grid_dim_y; j++){
          ...
          if (fil_id < NFIL){
            ...
            filament_references[3*fil_id + 2] = height_of_base_above_surface;
    ...
    const double Rsphere = double(R_OVER_L)*L + 1.05*(0.5*DL + RBLOB);

    #if EQUATORIAL_SEEDING

      std::ifstream input_file("sphere"+std::to_string(NFIL)+"_equatorial.seed");

    #elif UNIFORM_SEEDING

      std::ifstream input_file("sphere"+std::to_string(NFIL)+".seed");

    #elif PLATY_SEEDING

      std::ifstream input_file("sphere"+std::to_string(NFIL)+"_platy.seed");
    ...
    if (input_file.good()){
      ...
      for (int i = 0; i < 3*NFIL; i++){
        ...
        input_file >> filament_references[i];

      }

    } else {

      repulsive_filament_seeding(&filament_references[0]);
    ...
    for (int i = 0; i < NFIL; i++) {
      ...
      const double *const dir = &filament_references[3*i];
      const double pos[3] = {body.x[0] + dir[0], body.x[1] + dir[1], body.x[2] + dir[2]};
